...
func testUserToManyRoles(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"user_role\" (\"user_id\", \"role_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_role\" (\"user_id\", \"role_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Email`: `character varying`, `HashedPassword`: `character varying`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
